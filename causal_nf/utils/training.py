...
def load_normalizing_flow(cfg, preparator, ckpt_file=None):
    ...
    if cfg.model.adjacency:
        adjacency = preparator.adjacency(False)
    else:
        adjacency = None
...
def load_trainer(
    ...
    # NSF splines (zuko RQS) use torch.cumsum on CUDA, which has no kernel under
    # torch.use_deterministic_algorithms(True) (enabled when Trainer deterministic=True).
    layer_name = getattr(cfg.model, "layer_name", None)
    trainer_deterministic = layer_name != "nsf"
