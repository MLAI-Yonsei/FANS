    ...
    def __init__(self, sem_name):
        ...
        if sem_name == "linear":
            ...
            functions = [
                lambda u1: u1,
                lambda x1, u2: u2,
                lambda x1, x2, u3: (4.0 / (1 + torch.exp(-x2)) - x2**2) * u3,  # x1 removed
                lambda x1, x2, x3, u4: 20.0 / (1 + torch.exp(x3**2 / 2 - x3)) + u4,  # x1 not used originally
            ]
            inverses = [
                lambda x1: x1,
                lambda x1, x2: x2,
                lambda x1, x2, x3: x3 / (4.0 / (1 + torch.exp(-x2)) - x2**2),  # x1 removed
