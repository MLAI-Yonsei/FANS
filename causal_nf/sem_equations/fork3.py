    ...
    def __init__(self, sem_name):
        ...
        if sem_name == "linear":
            ...
            functions = [
                lambda u1: u1,
                lambda x1, u2: x1 + u2,  # x2 = x1 + u2
                # x3 = softplus(sin(x1²) + sin(x2²) + u3)
                lambda x1, x2, u3: torch.nn.functional.softplus(
                    torch.sin(x1**2) + torch.sin(x2**2) + u3
                ),
            ...
            inverses = [
                lambda x1: x1,
                lambda x1, x2: x2 - x1,  # u2 = x2 - x1
                # Inverse: u3 = ln(exp(x3) - 1) - sin(x1²) - sin(x2²)
                lambda x1, x2, x3: torch.log(torch.exp(x3) - 1) - torch.sin(x1**2) - torch.sin(x2**2),
