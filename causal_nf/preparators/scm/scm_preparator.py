    ...
    def get_scaler(self, fit=True):
        ...
        if fit:
            ...
            if self.scale in ["default", "std"]:
                scale = x.std(0)
                # Check for zero scale values and log to file
                zero_scale_mask = (scale == 0)
                if zero_scale_mask.any():
                    with open('zero_scale_log.txt', 'a') as f:
                        f.write(f"Zero scale values detected at indices: {torch.where(zero_scale_mask)[0].tolist()}\n")
                        f.write(f"Original scale values: {scale.tolist()}\n")
                        f.write(f"Timestamp: {datetime.now()}\n\n")
                
                scale = torch.where(scale == 0, torch.ones_like(scale), scale)
                self.scaler_transform = StandardTransform(
                    shift=x.mean(0), scale=scale
                )
