...
def run_linearccp_analysis(data_env1, data_env2, adj, metadata):
    ...
    for node in shifted_nodes:
        node_str = str(node)
        if node in functional_change_nodes:
    ...
    results = {
        ...
        "estimated_shift_types": estimated_shift_types,  # unified naming
        "functional_change_nodes": functional_change_nodes,  # kept for backward compatibility
        "noise_change_nodes": noise_change_nodes,  # kept for backward compatibility
