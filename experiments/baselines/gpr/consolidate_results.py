...
def consolidate_results(base_dir: str = "results/shift_detection"):
...
def consolidate_dataset(dataset_num: int, files: list, output_dir: Path):
    ...
    shifted_nodes = sorted([
        ...
        if result['true_shift'] in ['function', 'noise']
    ])
    ...
    for node in shifted_nodes:
        ...
        if true_shift == 'function':
            shift_types[str(node)] = ['fucntion']
        elif true_shift == 'noise':
            shift_types[str(node)] = ['noise']
