...
cat("=================================================================\n")
cat("Simultaneous Shift Detection - All Nodes\n")
cat("=================================================================\n\n")

np <- import("numpy", convert = FALSE)
node_sizes <- c("nodes_10")
graph_types <- c("ER")
data_base <- "/data1/statduck/data_both/data_violation_small/lambda_1.00"
results_base <- "results/shift_detection_simultaneous_violation"
threshold <- 10^(-5)
  ...
  for (graph_type in graph_types) {
    ...
    for (dataset_idx in 1:30) {
      ...
      for(node in all_shifted) {
        ...
        has_simultaneous_shift <- ("function_shift" %in% labels) && (length(labels) > 1)
        if(has_simultaneous_shift) {
          simultaneous_shifted <- c(simultaneous_shifted, node)
        } else {
      ...
      for (node_idx in shifted_nodes) {
        ...
        if(node_idx %in% function_shifted) {
          true_shift <- "function"
        } else if(node_idx %in% simultaneous_shifted) {
          true_shift <- "simultaneous"
        } else {
          true_shift <- "none"
        }
        ...
        tryCatch({
          ...
          if(length(parents) == 0) {
            ...
            var.equal <- FALSE
            cov.alter.unequal <- cov.structure(Z, kernel.result, var.equal)
            est.H1.unequal <- mle.H1(Y_combined, cov.alter.unequal, var.equal, est.H0, threshold)
            result$H1.unequal <- list(loglik = est.H1.unequal$log.lik, converged = est.H1.unequal$lik.diff <= threshold)

            delta_H1 <- est.H1$log.lik - est.H0$log.lik
            delta_H1_unequal <- est.H1.unequal$log.lik - est.H0$log.lik
            result$shift_detected <- if(delta_H1_unequal > delta_H1) "simultaneous" else "function"
