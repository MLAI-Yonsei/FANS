...
class FANSVisualizer:
    ...
    @staticmethod
    def plot_conditional_prob_comparison(plot_data, save_dir):
        """
        Visualize conditional probability comparison from precomputed KDE/JS data.

        All KDE evaluation and JS divergence computation lives in
        ``FANSAnalyzer._compute_conditional_prob_js``; this method only renders.

        Args:
            plot_data: dict produced by ``FANSAnalyzer._compute_conditional_prob_js``
                with ``return_plot_data=True``. Each value carries node_idx, quantiles,
                avg_js_divergence, shift_detected, js_threshold and a list of
                quantile_plots holding KDE arrays.
            save_dir: Directory to save plots.
        """
        if not plot_data:
            return

        for key, payload in plot_data.items():
            node_idx = payload['node_idx']
            quantiles = payload['quantiles']
            quantile_plots = payload['quantile_plots']
            ...
            for plot_info in quantile_plots:
                q_idx = plot_info['q_idx']
                q_value = plot_info['q_value']
                js_div = plot_info['js_div']
                kde = plot_info.get('kde')

                # Degenerate-variance quantiles have no KDE to draw
                if kde is None:
                    continue

                plt.subplot(n_rows, n_cols, q_idx + 1)
                plt.plot(kde['eval_range'], kde['pdf1_norm'], 'b-', label='Env1')
                plt.plot(kde['eval_range'], kde['pdf2_norm'], 'r-', label='Env2')

                plt.title(f'Quantile {q_idx} (p={q_value}): JS={js_div:.4f}')
                plt.xlabel('Value')
                plt.ylabel('Density')
                plt.legend(loc='best')
                ...
                if js_div > js_threshold:
                    plt.axvspan(kde['min_val'], kde['max_val'], alpha=0.1, color='red')
    ...
    def plot_dcor_independence_test(parent_data_env1, parent_data_env2, node_noise_env1, 
        ...
        plt.text(0.1, 0.5, f'DCor p-value: {env1_test_result["p_value"]:.4f}', 
                transform=plt.gca().transAxes, fontsize=14)
        ...
        plt.text(0.1, 0.7, f'DCor Score: {env2_test_result["dcor_score"]:.4f}', 
                transform=plt.gca().transAxes, fontsize=14)
        plt.text(0.1, 0.5, f'DCor p-value: {env2_test_result["p_value"]:.4f}', 
                transform=plt.gca().transAxes, fontsize=14)
